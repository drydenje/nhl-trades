...
var _teamNhlId = require("./player-data/team-nhl-id");
...
const headings = ["id", "name", "abbreviation", "nicknames", "colors", "logo", "goalHorn",
// "goalHornSong",
"isActive", "start", "end"];
const temp = _teamNhlId.teams.map(team => {
...
console.log(temp[0]);
const csv = (0, _convertArrayToCsv.convertArrayToCSV)(temp, {
  headings,
  separator: ","
});

// writeFile(`./src/player-data/nhl-team-ids.csv`, csv);
_fs.default.writeFile("./src/csv-data/nhl-team-ids.csv", csv, err => {
  if (err) {
    console.log(err);
  }
});
