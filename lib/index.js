...
const cleanData = () => {
  ...
  for (const [key, value] of Object.entries(arr)) {
    ...
    arr[key].forEach(item => {
      ...
      const lineItem = `${item.playerName}` + ",\n/n" + `${item.draftYear},` + `${item.id},` + `${item.triCode},` + `${item.teamPickHistory},` + `${item.supplementalDraft},` + `${item.overallPickNumber},` + `${item.pickInRound},` + `${item.notes},` + `${item.draftedByTeamId},` + `${item.draftDate}`;
