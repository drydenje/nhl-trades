...
var _nhlBirthdayScraper = require("./utils/nhl-birthday-scraper");
...
const scrapeNHLBirthdays = async () => {
  ...
  const players = JSON.parse(_fs.default.readFileSync(DUPLICATE_IDS, "utf8")
  // , (err, data) => {
  //   if (err) throw err;
  //   console.log(data);
  // })
  ...
  // console.log(players);
  const newJSON = await (0, _nhlBirthdayScraper.addBirthdayToPlayer)(players);
  const remaining = newJSON.filter(player => player.birthDate === undefined).length;
  console.log("Remaining:", remaining);
  _fs.default.writeFileSync(`./public/scraped-data/duplicate-ids-copy.json`, JSON.stringify(newJSON), (err, data) => {
    if (err) throw err;
    console.log("HUH", data);
  });
};
_nodeCron.default.schedule("*/5 * * * * *", scrapeNHLBirthdays);
