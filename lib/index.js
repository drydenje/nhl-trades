...
var _nhlDraftScraping = require("./nhl-draft-scraping.js");
var _fs = _interopRequireDefault(require("fs"));
var _nodeCron = _interopRequireDefault(require("node-cron"));
var _playerNhlId = require("./player-nhl-id.js");
var _res = require("./res.js");
var _hrPlayerIdCopy = require("./hr-player-id-copy.js");
...
// // import { hdbPlayers } from "../public/raw-mock-data/hockeydb/res.js";
// 183 duplicates
const duplicatePlayers = JSON.parse(_fs.default.readFileSync("./src/duplicate-ids-copy2.json", "utf8"));
...
const findHdbPlayer = name => {
  return _res.hdbPlayers.find(player => player.name === name);
};

// console.log(findHdbPlayer("Spencer Abbott"));

const findHrPlayer = name => {
  return _hrPlayerIdCopy.hrPlayers.find(player => player.name === name);
};
const addDuplicatePlayers = async () => {
  // console.log("Duplicate players count:", duplicatePlayers.length);

  const newPlayerArray = _playerNhlId.players.map(player => {
  ...
  // "id": 8481124,
  // "name": "Zachary Magwood",
  // "hrID": "magwoza01",
  // "birthDate": null,
  // "hdbID": null

  console.log("Total Players:", newPlayerArray.length);
  console.log("Players without hrID's:", newPlayerArray.filter(player => player?.hrID === null).length);
  console.log("Players with hrID's:", newPlayerArray.filter(player => player?.hrID !== null).length);
  console.log("Players without birthDates:", newPlayerArray.filter(player => player?.birthDate === null).length);
  console.log("Players without hdbID's:", newPlayerArray.filter(player => player?.hdbID === null).length);
  _fs.default.writeFileSync(`./src/player-id.json`, JSON.stringify(newPlayerArray), err => {
    if (err) throw err;
