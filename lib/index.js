...
_playerNhlId.players.forEach(player => {
  const test = _hrPlayerIdCopy.hrPlayers.filter(hrPlayer => hrPlayer.name === player.name);
  const obj = {
    id: player.id,
    name: player.name,
    ...(test[0] ? {
...
_hrPlayerIdCopy.hrPlayers.forEach(player => {
  // Check if the player exists in the final array
  const filteredPlayer = _playerNhlId.players.filter(nhlPlayer => {
