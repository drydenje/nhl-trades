...
exports.writeFile = exports.removeNickname = exports.readFile = exports.getPage = exports.getNextYear = exports.combineSiteIds = exports.checkArray = void 0;
...
const combineSiteIds = (nhlPlayers, hdbPlayers, hrPlayers) => {
  ...
  const playerResult = players.map((player, index) => {
    ...
    const hr = hrIDs.find(hrPlayer => checkNameAndBirthDate(player, hrPlayer) || checkNameAndCity(player, hrPlayer));
    const hdb = hdbIDs.find(hdbPlayer => checkNameAndBirthDate(player, hdbPlayer) || checkNameAndCity(player, hdbPlayer));
...
const checkNameAndCity = ({
  ...
  return `${firstName.default} ${lastName.default}` === hName && birthCity === hBirthCity;
};
