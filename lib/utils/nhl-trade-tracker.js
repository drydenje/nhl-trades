...
exports.parseTrade = exports.getTradesFromPage = exports.getSeasonList = exports.getPageCount = exports.getAllTradesForYear = exports.checkMaxUserException = void 0;
...
//     "comment": null
// }
exports.getTradesFromPage = getTradesFromPage;
const parseTrade = html => {
  let $ = cheerio.load(html);
  const headings = $("tbody > tr").find("td > strong").map(function () {
    if ($(this).text() !== "Date") {
  ...
  const roster = [];
  for (let i = 0; i <= 1; i++) {
    roster[i] = $(`table:nth(${i + 1})`).text().trim().split("\n").map(item => {
      ...
      const regexTextBetweenQuotes = /(?<=(["']\b))(?:(?=(\\?))\2.)*?(?=\1|\|)/g;
      playerID ? player.hockeyDBid = playerID.match(regexTextBetweenQuotes)[0] : "0";
      return player;
    }).filter(s => s.name);
  }
...
const getSeasonList = page => {
  let $ = cheerio.load(page);
  const arr = $(".s_list > tbody").find("tr").text().split("\n").filter(s => s).map(s => s.trim());
...
const getAllTradesForYear = async (year, baseUrl) => {
  ...
  pages.forEach((page, i) => {
    ...
    tasks.push(new Promise(async function (resolve) {
      ...
      let result = await (0, _utils.getPage)(page).then(page => {
  ...
  if (pageCount === results.length) {
    ...
    console.log(chalk.black.bgRed(`Didn't scrape all of the pages for: ${year}`));
    throw new Error("Failed to scrape data");
  }
};

// this error should fire when too many requests are made too quickly
exports.getAllTradesForYear = getAllTradesForYear;
...
exports.checkMaxUserException = checkMaxUserException;